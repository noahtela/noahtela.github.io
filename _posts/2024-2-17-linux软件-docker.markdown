---
layout:     post
title:      "linux软件-docker"
subtitle:   " \"linux\""
date:       2024-2-17 17:22:49
author:     "yangsir"
...
![image-20240217141730504](\img\springBoot\image-20240217141730504.png)
...
![image-20240217141809333](\img\springBoot\image-20240217141809333.png)
...
```
docker save -o 存出镜像为本地文件 镜像名
```

将存出的镜像从A 机器拷贝到B 机器，需要在B 机器上使用该镜像，就可以将该导出文

件导入到B 机器的本地镜像列表中，这一过程叫做载入镜像。使用docker load 或者docker load --input 进行载入操作。

```
docker load < 存出的文件或者docker load --input 存出的文件
```
...
例：导出**21f5e00a42e7**容器到文件centos-exp：

```
docker export 21f5e00a42e7 > centos-exp
```
...
例：导入文件centos7.tar成为本地镜像:

```
cat centos-exp | docker import - centos:exp
```
...
| CMD[“要运行的程序”,"参数 1”,"参数 2”] | 设置指令，用于 container(容器)启动时指定的操作。             |
